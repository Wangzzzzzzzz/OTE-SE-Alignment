...
from memit.memit_seq_prune_main import apply_memit_seq_prune_to_model
from memit.memit_seq_prune_err_main import apply_memit_seq_prune_err_to_model
...
from memit.memit_forgetting_main import apply_memit_forgetting_to_model
...
ALG_DICT = {
    ...
    "MEMIT_seq_prune": (MEMITHyperParams, apply_memit_seq_prune_to_model),
    "MEMIT_seq_prune_err": (MEMITHyperParams, apply_memit_seq_prune_err_to_model),
    ...
    "MEMIT_forgetting": (MEMITHyperParams, apply_memit_forgetting_to_model),
...
def main(
    ...
    eval_ds = ds_class(DATA_DIR, tok=tok, size=dataset_size_limit+1000)
    eval_ds = eval_ds[-1000:]
    ...
    cache_template = None
    if use_cache:
        if any(alg in alg_name for alg in ["MEMIT","AlphaEdit", "MEMIT_seq", "MEMIT_prune", "MEMIT_seq_prune", "MEMIT_rect"]):
            cache_template = (
                KV_DIR
    ...
    if any(alg in alg_name for alg in ["AlphaEdit", "MEMIT_seq", "MEMIT_prune", "NSE"]):
        ...
        if hparams.model_name == "gpt2-xl":
            ...
            error_cache = torch.zeros((len(hparams.layers), W_out.shape[1], W_out.shape[0]), device='cpu', dtype=torch.double)
            if alg_name == "AlphaEdit":
                P = torch.zeros((len(hparams.layers), W_out.shape[0], W_out.shape[0]), device="cpu")
        elif hparams.model_name in ["EleutherAI_gpt-j-6B","Llama3-8B","phi-1.5","Qwen2.5-7B"]:
    ...
    hidden_save_location = str(run_dir) + '/' + 'hidden_result/'
    os.makedirs(hidden_save_location, exist_ok=True)
    
    hs = get_module_input_output_at_words(
            model,
            tok,
            hparams.layers[-1],
            context_templates=[request["prompt"] for request in eval_ds],
            words=[request["subject"] for request in eval_ds],
            module_template=hparams.layer_module_tmp,
            fact_token_strategy=hparams.fact_token,
        )[1].T
    torch.save(hs, hidden_save_location+"pre_edit_hs.pt")
    # del hs


    cnt = 0
    cumulative_args = dict(total_upd_matrix={})  
    cov_matrix_args = dict(total_cov_matrix={})  
    forgetting_args = dict(total_upd_matrix={})  
    for record_chunks in chunks(ds, num_edits):
        ...
        error_args = dict(error_cache=error_cache) if any(alg == alg_name for alg in ["MEMIT_seq_rect", "MEMIT_seq_rect_err", "MEMIT_seq_prune", "MEMIT_seq_prune_err"]) else dict()
        # cumulative_args = dict()
        # === End of loop iteration: update cumulative_args if needed ===
        

        if (cnt == 0 and args.downstream_eval_steps > 0) and args.eval_glue:#do initial GLUE EVAL WITH ORIGINAL MODEL
        ...
        start = time()
        if any(alg == alg_name for alg in ["AlphaEdit", "MEMIT_seq", "NSE"]):
        ...
        elif any(alg == alg_name for alg in ["MEMIT_seq_rect", "MEMIT_seq_rect_err", "MEMIT_seq_prune", "MEMIT_seq_prune_err"]):
            edited_model, cache_c, error_cache = apply_algo(
                model,
                tok,
                [
                    {"case_id": record["case_id"], **rewrite_dict}
        ...
        elif any(alg == alg_name for alg in ["MEMIT_forgetting"]):
            edited_model, cache_c, total_upd_matrix, total_cov_matrix = apply_algo(
                model,
                tok,
                [
                    {"case_id": record["case_id"], **rewrite_dict}
            ...
            forgetting_args = dict(total_upd_matrix=total_upd_matrix)
        ...
        if (args.downstream_eval_steps > 0 and cnt % args.downstream_eval_steps == 0) and args.eval_glue:
            glue_results = {
    ...
    torch.save(hs, hidden_save_location+"post_edit_hs_memit.pt")
    start = time()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--alg_name",
        choices=["AlphaEdit","MEMIT_rect", 
                 "MEMIT_seq","MEMIT_prune", "MEMIT_seq_prune", "MEMIT_seq_rect", 
                 "MEMIT_seq_rect_err", "MEMIT_seq_prune_err",
                 "MEMIT", "ROME", "FT", "MEND","NSE", "MEMIT_forgetting"],
        default="ROME",
    ...
    parser.add_argument(
        "--normalization",
        action="store_true",
        help="Whether to normalize update matrices during MEMIT optimization.",
    )

    parser.add_argument(
        "--cache_c",
        action="store_true",
        help="histrorical infomartion.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Weight used when updating the second moment estimator in MEMIT.",
    )
